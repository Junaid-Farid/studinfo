		...
		confirm_query($result);
					while ($row = mysql_fetch_array($result)) {
						...
						echo (@$odd == true) ? ' <tr class="odd_row" > ' : ' <tr class="even_row"> ';
							@$odd = !$odd;
						echo ' <td> ';
						echo '<a href="edit_department.php?id='.$row['dept_id'].'" class="app_listitem_key">'.$row['dept_name'].'</a>';
						echo ' <td> ';
						echo $row['dept_description'];
						if ($_SESSION['usertype']=='Administrator' || $_SESSION['usertype']=='Registrar'){
							echo ' <td> ';
							echo '<a href="process_del_department.php?id='.$row['dept_id'].'" class="app_listitem_key">[Delete Entry]</a>';
