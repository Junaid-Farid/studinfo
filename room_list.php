...
<div id="module-name">List of Rooms</div>
		...
		confirm_query($result);
					while ($row = mysql_fetch_array($result)) {
						...
						echo (@$odd == true) ? ' <tr class="odd_row" > ' : ' <tr class="even_row"> ';
							@$odd = !$odd;
						echo ' <td> ';
						echo '<a href="edit_room.php?id='.$row['room_id'].'" class="app_listitem_key" title="Edit">'.$row['room_name'].'</a>';
						echo ' <td> ';
						echo $row['room_desc'];
						if ($_SESSION['usertype']=='Administrator' || $_SESSION['usertype']=='Registrar'){
							echo ' <td> ';
							echo '<a href="process_del_room.php?id='.$row['room_id'].'" class="app_listitem_key" title="Delete"><img src="img_icon\Trash.ico" width="30" height="30"/></a>';
